...
(() => {
  const LOCAL_STORAGE_KEY = 'isyancem:image-slots:v1';
  const MAX_DIM = 1200;
  ...
  function writeLocalStore() {
    try {
      if (window.localStorage) {
        window.localStorage.setItem(LOCAL_STORAGE_KEY, JSON.stringify(slots));
      }
    } catch { }
  }

  function mergeSlotState(j) {
    if (j && typeof j === 'object') {
      const merged = Object.assign({}, j, slots);
      // A framing-only write that raced ahead of hydration must not drop a user
      // image that's only on disk/localStorage — inherit u for in-memory entries.
  ...
  function load() {
    ...
    loadP = Promise.resolve()
      .then(() => {
        // Next.js: usamos solo localStorage, no archivo .json
        mergeSlotState(readLocalStore());
        tombstones.clear();
      })
      .catch(() => { })
      .then(() => {
